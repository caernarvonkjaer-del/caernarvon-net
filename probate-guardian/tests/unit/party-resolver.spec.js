// The Party hydration/dehydration, de-duplication, merge/unmerge and
// ward-identity core (src/core/party-resolver.js; persistence-rewrite
// Milestones 3, 7 and 49B), exercised directly against hand-built parties and
// filings.
//
// Moved here from tests/e2e/party-resolver.spec.ts by Milestone 70's 70T.
// Every one of these tests ran a single page.evaluate() body against the
// modules' window bridges, and many depend on object identity (resolveParty()
// returning the very party createParty() made, merges repointing live
// filings) that a browser spec reaching the app only through
// GuardianForms.testing's copies cannot observe. Nothing here needed a
// browser. Each body is unchanged: `w` resolves the modules' own exports
// first and the stubbed window second, and window.caseFile is the empty case
// the app itself starts from. The three tests that package or open a .sav
// stay in the browser spec.
...
describe('party-resolver (unwired hydration/dehydration core)', () => {
  test('createParty + resolveParty round-trip, and resolveParty follows a merge tombstone', () => {
  ...
  test('identitySlotForPath recognizes array/object/flat identity paths and ignores everything else', () => {
  ...
  test('identitySlotForPath covers the remaining Milestone 5 types (simplified, planSimplified, planAnnual, planMinor)', () => {
  ...
  test('guardian (Initial Inventory): guardian row, nested attorney object, and nested preparer object all round-trip', () => {
  ...
  test('simplified: guardian row and flat attorney fields round-trip; this type has no preparer mapping', () => {

    const result = (() => {
      const w = api;
      const filing = { inventoryType: 'simplified', wardName: 'Test Ward', guardians: [{}] };
  ...
  test('annual: guardian row (with office address), flat attorney (attorney_bar), nested preparer object round-trip', () => {

    const result = (() => {
      const w = api;
      const filing = { inventoryType: 'annual', wardName: 'Test Ward', guardians: [{}] };
  ...
  test('planSimplified: guardian row address round-trips through the single joined mailingAddress field', () => {

    const result = (() => {
      const w = api;
      const filing = { inventoryType: 'planSimplified', wardName: 'Test Ward', planGuardians: [{}] };
  ...
  test('planAnnual: guardian row (with office address) and scalar-named attorney round-trip', () => {

    const result = (() => {
      const w = api;
      const filing = { inventoryType: 'planAnnual', wardName: 'Test Ward', planGuardians: [{}] };
  ...
  test('planMinor: guardian/preparer taxId round-trips through the tin field name', () => {

    const result = (() => {
      const w = api;
      const filing = { inventoryType: 'planMinor', wardName: 'Test Ward', planGuardians: [{}] };
  ...
  test('dehydrateIntoParty merges a filing role’s current fields onto the party’s canonical shape', () => {

    const result = (() => {
      const w = api;
      const party = w.createParty('attorney');
      const filing = { inventoryType: 'planInitial', wardName: 'Test Ward' };
  ...
  test('syncIdentityField propagates one edit across every other filing/slot sharing the same party', () => {
    ...
    const result = (() => {
      ...
      const filingA = { inventoryType: 'guardian', wardName: 'Ward A', attorney: {}, attorneyPartyId: attorneyParty.id };
      const filingB = { inventoryType: 'simplified', wardName: 'Ward B', attorneyPartyId: attorneyParty.id };
  ...
  test('syncIdentityField is a no-op when the slot has no party attached', () => {

    const result = (() => {
      const w = api;
      const filing = { inventoryType: 'guardian', wardName: 'Ward A', attorney: { name: 'Rob Atty' } };
...
describe('party de-duplication via resolver (Milestone 7)', () => {
  test('findDuplicateCandidates matches on exact case-insensitive name, flags strongMatch on shared contact info, and excludes dismissed pairs', () => {
    ...
    const result = (() => {
      ...
      return {
        ...
        strongPair: after.find((c) => [c.partyA.id, c.partyB.id].includes(strong.id)),
  ...
  test('isPartyPairDismissed is order-independent', () => {
  ...
  test('mergeParties repoints every FK across multiple wards and a case, unions roles, and tombstones the discarded party', () => {
    ...
    const result = (() => {
      ...
      const wardA = { wardId: 'a', inventoryType: 'guardian', guardianPartyIds: [discard.id], attorney: {} };
      const wardB = { wardId: 'b', inventoryType: 'annual', guardianPartyIds: [null, discard.id], attorney_bar: '', attorney: '' };
  ...
  test('mergeParties never overwrites a field keep already has, and adoptBlankFields:false skips backfill entirely', () => {
  ...
  test('legacy backfill in .sav import: unanimous linked counties persist, conflicting linked counties leave county blank', () => {
...
describe('party merge undo & near-match detection', () => {
  test('findDuplicateCandidates lists exact-name pairs first, then near-name pairs, and leaves genuinely different names alone', () => {

    const result = (() => {
      const w = api;
      const make = (name) => { const p = w.createParty('guardian'); p.name = name; return p; };
      ...
      const ids = { a: a.id, b: b.id, c: c.id, d: d.id, e: e.id, f: f.id, g: g.id, h: h.id };
      const label = (id) => Object.keys(ids).find(k => ids[k] === id);
      const candidates = w.findDuplicateCandidates();
      return {
        kinds: candidates.map((x) => `${[label(x.partyA.id), label(x.partyB.id)].sort().join('')}:${x.matchKind}`),
  ...
  test('mergeParties records what it changed, and unmergeParty puts it back: FKs return to the sub and re-hydrate, adopted fields clear, adopted roles drop', () => {
    ...
    const result = (() => {
      ...
      const wardA = { wardId: 'a', inventoryType: 'guardian', guardianPartyIds: [sub.id], attorney: {} };
      const kase = w.createCase({ caseNumber: '24-1', county: 'Pinellas' });
      ...
      const afterMerge = {
        listedUnderPrimary: w.subPartiesOf(primary.id).map((p) => p.id),
  ...
  test('unmergeParty keeps a primary field edited since the merge, and leaves a slot that was re-linked elsewhere since the merge', () => {
    ...
    const result = (() => {
      ...
      const wardA = { wardId: 'a', inventoryType: 'guardian', guardianPartyIds: [sub.id] };
      cf.wards.push(wardA);
  ...
  test('one level only: a sub of a sub stays hidden under the new primary and cannot be unmerged until the middle record is; a pre-tracking tombstone is never listed', () => {
    ...
    const result = (() => {
      ...
      const underCBefore = w.subPartiesOf(c.id).map((p) => p.id);
      const aWhileNested = w.unmergeParty(a.id);
      const resolvedA = w.resolveParty(a.id)?.id;

      const bUnmerged = w.unmergeParty(b.id);
      const underCAfter = w.subPartiesOf(c.id).map((p) => p.id);
      const underBAfter = w.subPartiesOf(b.id).map((p) => p.id);
...
describe('ward identity role & closed-filing cut-off (Milestone 49B)', () => {
  test('ward fields round-trip per type: Annual Plan (SSN, residence, mailing), Minor Plan (split city/state/zip), Simplified (SSN), Guardian Inventory (name only)', () => {
    ...
    const result = (() => {
      ...
      const planAnnual = { inventoryType: 'planAnnual' };
      const planMinor = { inventoryType: 'planMinor' };
      const simplified = { inventoryType: 'simplified' };
      const inventory = { inventoryType: 'guardian' };
      ...
      const minorEdited = { inventoryType: 'planMinor', wardName: 'Eleanor Whitfield', q1Street: '12 Oak St', q1City: 'St. Petersburg', q1State: 'FL', q1Zip: '33701', q1Phone: '(727) 555-0202' };
  ...
  test('a closed filing neither receives party edits nor sends its own out; drift is reported and cleared by an explicit sync; reopening does not catch up by itself', () => {
    ...
    const result = (() => {
      ...
      const open = { wardId: 'open', inventoryType: 'annual', guardianPartyIds: [party.id] };
      const closed = { wardId: 'closed', wardName: 'Ward C', inventoryType: 'planInitial', guardianPartyIds: [party.id], archived: true };
      ...
      const drift = w.closedFilingDrift(party.id).map((d) => ({ wardId: d.filing.wardId, role: d.role, keys: d.differences.map((x) => x.key).sort() }));
      const filingDrift = w.filingDriftFromParties(closed).map((d) => ({ role: d.role, partyName: d.party.name }));
  ...
  test('legacy backfill fills the ward Party from its filings and open filings from the Party, never overwriting a value or touching a closed filing', () => {

    const result = (() => {
      const w = api;
      const cf = w.caseFile;
      const party = w.createParty('ward'); party.name = 'Eleanor Whitfield'; // pre-49B: name and county only
      const annualPlan = { wardId: 'ap', inventoryType: 'planAnnual', wardPartyId: party.id, wardName: 'Eleanor Whitfield', ssn: '111-22-3333', residenceAddress: '410 Bayshore Dr', residenceCityStateZip: 'Clearwater, FL 33756', residencePhone: '', mailingAddress: '', mailingCityStateZip: '' };
      const initialPlan = { wardId: 'ip', inventoryType: 'planInitial', wardPartyId: party.id, wardName: 'Eleanor Whitfield', residenceAddress: '', residenceCityStateZip: '', residencePhone: '(727) 555-0101', mailingAddress: 'PO Box 9', mailingCityStateZip: 'Largo, FL 33770' };
      const closedPlan = { wardId: 'cp', inventoryType: 'planAnnual', wardPartyId: party.id, wardName: 'Eleanor Whitfield', ssn: '', residenceAddress: '99 Old Rd', residenceCityStateZip: 'Tampa, FL 33602', residencePhone: '', archived: true };
      const inventory = { wardId: 'gi', inventoryType: 'guardian', wardPartyId: party.id, wardName: 'Eleanor Whitfield' };
  ...
  test('merge and unmerge move a closed filing\'s link but leave its copy alone, so it shows as drift', () => {
    ...
    const result = (() => {
      ...
      const closed = { wardId: 'closed', wardName: 'Ward C', inventoryType: 'annual', guardianPartyIds: [sub.id], archived: true };
      cf.wards.push(closed);
      w.hydrateFromParty(sub, closed, 'guardian', 0);

      w.mergeParties(primary.id, sub.id, { adoptBlankFields: false });
      const afterMerge = { link: closed.guardianPartyIds[0], phone: closed.guardians[0].phone, drift: w.closedFilingDrift(primary.id).map((d) => d.differences.map((x) => x.key)) };
  ...
  test('editing one identity field promotes only that field, and corrects the stale block it came from', () => {
