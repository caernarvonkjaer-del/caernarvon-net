...
async function setPeriod(page: Page, from: string, to: string) {
  await page.evaluate(([f, t]) => { const w = window as any; w.GuardianForms.testing.patchFiling({ 'periodFrom': f }); w.GuardianForms.testing.patchFiling({ 'periodTo': t }); w.GuardianForms.testing.save.auto(); }, [from, to]);
