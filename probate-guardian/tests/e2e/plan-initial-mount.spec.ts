...
registerPlanMountTests({
  ...
  triggerExport: (page) => page.evaluate(() => (window as any).GuardianForms.testing.saveOutput.pdfPlanInitial()),
...
test('Milestone 58C: Primary Email\'s required marker follows the attorney block, live and in place', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    for (const k of ['attorney_name', 'attorney_bar', 'attorney_email', 'attorney_secondaryEmail',
      ...
      'attorney_signatureDate', 'attorney_signatureState']) w.GuardianForms.testing.patchFiling({ [k]: '' });
    w.GuardianForms.testing.navigate('/p10');
  });
