...
test('ward residence syncs between open Plans, stops for a closed one, and syncs on request from the record and from the Cover', async ({ page }) => {
  ...
  await page.evaluate(() => { const w = window as any; w.commitCoverCounty(w.D, 'Pinellas'); w.navigate('/'); });
  const filing1 = await page.evaluate(() => (window as any).D.wardId);
  ...
  const partyAfterFill = await page.evaluate(() => { const w = window as any; return w.resolveParty(w.D.wardPartyId).address; });
  ...
  const filing2 = await page.evaluate(() => (window as any).D.wardId);
  expect(await page.evaluate(([a, b]) => { const w = window as any; const f = (id: string) => w.caseFile.wards.find((x: any) => x.wardId === id); return f(a).wardPartyId === f(b).wardPartyId; }, [filing1, filing2])).toBe(true);
  ...
  expect(await page.evaluate((id) => (window as any).caseFile.wards.find((x: any) => x.wardId === id).residencePhone, filing1)).toBe('(727) 555-0101');
  ...
  await page.evaluate((id) => (window as any).switchWard(id), filing2);
  await page.evaluate(() => (window as any).navigate('/'));
  ...
  expect(await page.evaluate((id) => (window as any).caseFile.wards.find((x: any) => x.wardId === id).residencePhone, filing1)).toBe('(727) 555-0202');
  ...
  await page.evaluate((id) => (window as any).switchWard(id), filing1);
  await page.evaluate(() => (window as any).navigate('/'));
  ...
  await page.locator('[data-form-path="residencePhone"]').blur();
  expect(await page.evaluate((id) => (window as any).caseFile.wards.find((x: any) => x.wardId === id).residencePhone, filing1)).toBe('(727) 555-0404');
});
