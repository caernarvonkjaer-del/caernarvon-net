...
test.describe('Plan benefit answers are explicit Yes/No/Unanswered radios', () => {
  test('Plan Annual benefit table starts unanswered and stores both explicit answers', async ({ page }) => {
    ...
    expect(await page.evaluate(() => (window as any).GuardianForms.testing.field('benefits.socialSecurity.eligible'))).toBe('No');
    ...
    expect(await page.evaluate(() => (window as any).GuardianForms.testing.field('benefits.other.appliedFor'))).toBe('Yes');
  });
  ...
  test('Plan Initial Question 7 starts unanswered and stores No without triggering a Yes-only explanation', async ({ page }) => {
    ...
    expect(await page.evaluate(() => ({
      trusts: (window as any).GuardianForms.testing.field('q7Trusts'),
      pending: (window as any).GuardianForms.testing.field('q7PendingBenefits'),
