...
test.describe('Supplemental PDF inline rendering for Accounting forms (Annual, Trust, Final)', () => {
  test('Annual Accounting inserts uploaded Schedule A supporting PDF directly after Schedule A', async ({ page }) => {
    ...
    const fixture = await buildSupplementalAttachmentFixture(page, 'SCHEDULE A ATTACHMENT: Social Security Benefit Verification Letter', {
      id: 'test-ssa-letter', name: 'SSA_Award_Letter_2026.pdf', size: 4200,
    });
  ...
  test('Trust Accounting alias inserts supporting documents in PDF preview and export', async ({ page }) => {
    ...
    const fixture = await buildSupplementalAttachmentFixture(page, 'TRUST ACCOUNTING SCHEDULE A: 1099-DIV Statement', {
      id: 'test-trust-1099', name: 'Trust_1099_2026.pdf', size: 4500,
    });
  ...
  test('Final Accounting alias correctly resolves and embeds supplemental PDF', async ({ page }) => {
    ...
    const fixture = await buildSupplementalAttachmentFixture(page, 'FINAL ACCOUNTING: Final Closing Bank Statement', {
      id: 'test-final-statement', name: 'Final_Closing_Statement.pdf', size: 3900,
    });
  ...
  test('Guardian Inventory multi-year regression: activeYearKey attachments continue to insert correctly', async ({ page }) => {
    ...
    const fixture = await buildSupplementalAttachmentFixture(page, 'GUARDIAN INVENTORY YEAR 2: Bank Statement Verification', {
      id: 'test-gi-y2', name: 'Y2_Checking.pdf', size: 3500,
    });
