 ...
 */
// Milestone 42F, transitional. The pre-42F way of finding a field path: pattern-
// matching the message's detail text, per filing type. Every validator now
// states its own path (see validation-issue.js), so this runs only for the
// bare-string issues that remain (e.g. getSupplementalFilingIssues()) and as
// the oracle tests/e2e/validation-structured-paths.spec.ts checks each
// validator's explicit paths against. Step 3 of 42F deletes it.
export function _legacyDerivePath(section, detail, formType) {
  let path = '';
...
export function adaptValidationErrors(errors = [], formType = 'guardian') {
  ...
  return errors.map((err) => {
    ...
    const path = _legacyDerivePath(section, detail, formType);
...
if (typeof window !== 'undefined') {
  ...
  window._legacyDerivePath = _legacyDerivePath;
