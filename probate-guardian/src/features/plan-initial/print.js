  ...
  return `<div>
    ...
    <div class="print-preview-banner no-print">
      ...
      <div class="d-flex gap-2 flex-wrap">
        ...
        <button class="btn btn-primary btn-sm" data-form-action="save-pdf-plan-initial" ${pdfBlocked?'disabled':''}>Save as PDF</button>
...
export async function doSavePdf(){
  ...
  const btn = beginExport('[data-form-action="save-pdf-plan-initial"]');
  if (!btn) return;
  const ward=(getD().wardName||'InitialGuardianshipPlan').replace(/[^a-z0-9]/gi,'_');
  try{
    const model = buildPlanInitialModel(getD());
