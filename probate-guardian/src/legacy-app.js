...
// form balances trivially at 0 = 0 and is not a discrepancy. Stays a legacy
// global alongside calcTotalsAnnual() (Milestone 7 plan's "Confirmed
...
// Interim duplicates of fmtD()/fmtAnnual()/DISB_CATS (moved to
// src/features/annual-accounting/index.js, Milestone 7, Phase A) --
// buildPrintHTMLAnnual()/doSaveExcelAnnual() below still need them until
// Phase B moves those functions into print.js/excel.js too, where they'll
// define their own copies (same pattern already used for e.g.
// src/features/plan-annual/print.js's local y()/line()/fld() helpers,
// rather than sharing tiny formatting closures across files). Delete these
// once Phase B lands.
...
function pagePrintAnnual(){
  const errors=window.validateAnnual();
...
// validateAnnual() moved to src/features/annual-accounting/index.js
// (Milestone 7, Phase A). Exported from there, called via window from the
// still-legacy functions below until Phase B moves them out too.
...
async function doSaveExcelAnnual(){
  const errors=window.validateAnnual(); if(errors.length){renderPage('/print');return;}
