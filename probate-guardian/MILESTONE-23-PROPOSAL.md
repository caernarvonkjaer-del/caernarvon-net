# Milestone 23: User-Attested Supplemental PDF Bundling
...
2. **User attestation required**: every uploaded supplemental PDF must carry an explicit user acknowledgement that the document is accessible and suitable for filing.
...
- Require attestation before export.
...
- Confirm that the uploaded documents are suitable for inclusion.
...
> Supplemental PDFs are inserted as uploaded. Probate Guardian does not certify or remediate uploaded documents for accessibility. By including a supplemental PDF, you confirm that it is accessible and suitable for filing.
...
Extend each supporting-document file record to distinguish the original upload, technical validation, and user attestation:
...
```js
{
  ...
  attestedDigest: "sha256-...",
  attestedAt: "2026-09-06T...",
...
| `attestationStatus` | `pending`, `accepted`, `revoked` | Whether the filer has accepted responsibility for the uploaded PDF. |

Do not migrate existing `.sav` files destructively. On load, legacy supporting files should be treated as `attestationStatus: "pending"` unless they already contain the new attestation fields.
...
3. Validation moves the record to exactly one terminal technical state:
   ...
   - `warning`
   - `blocked`
4. User attestation may be accepted only for `ready` or `warning` files.
5. Attestation binds to the exact `contentDigest` by writing `attestedDigest`.
6. Any replacement, re-upload, byte change, or digest mismatch revokes attestation and returns the file to `pending` or `checking`.
7. A stale validation result after removal or replacement is ignored.
8. If autosave fails during validation, the UI must surface the save failure and keep export blocked until the saved record and in-memory record agree.
9. On reload, `checking` records are treated as incomplete and validation is rerun from the stored original Data URL.
...
6. `attestationStatus` is `accepted`.
7. `attestedDigest` exactly equals `contentDigest`.
8. It is not marked encrypted, corrupt, removed, stale, or blocked.

Legacy records with missing status or attestation fields are ineligible in memory until the user reviews and attests them. The existing `dataUrl` test is not sufficient.
...
A scanned PDF with visible page images but no text objects is not empty for bundling purposes. It may be a poor accessibility candidate, but under this milestone the filer may include it only after attestation unless court technology-standard policy makes no-text PDFs a hard block in a later milestone.

## Implementation Slices

### Slice 23A: Upload Policy and Attestation UI
...
3. Add an attestation checkbox or confirmation step per uploaded file:
   - "I confirm this supplemental PDF is accessible and suitable for filing."
4. Show per-file status:
   - `Checking`
   - `Ready - attestation needed`
   - `Ready`
   - `Warning - review recommended`
   - `Blocked`
5. Keep the existing original download action.
6. Reject unsupported file types before saving them into the ward record.
7. Bind attestation to `contentDigest`, not only filename or file id.
...
4. Disable final export only when:
   ...
   - a supplemental PDF lacks required attestation.
5. Do not block export merely because a lightweight warning exists after the user has attested.
6. Treat missing attestation/status fields as `pending` during export readiness checks.
...
4. Update `docs/pdf-architecture-and-signatures.md` to reflect the new boundary:
   ...
   - uploaded supplemental PDFs are user-attested and preserved;
...
2. Mark legacy records `pending` for attestation on first edit/export.
...
5. Ensure `.sav` export/import round-trips the new status and attestation fields.
6. During export, treat absent legacy status or attestation fields as `pending` even before the record is rewritten.
...
6. Required user attestation before export.
7. Legacy `.sav` records loading as attestation-pending.
8. Legacy `.sav` records with only `dataUrl` are refused by the model/finalizer guard until attested.
...
15. Attestation revoked when `contentDigest` changes.
...
2. Each uploaded supplemental PDF has local technical status, page count, warnings if applicable, and explicit attestation state.
3. Export is blocked for unsupported, corrupt, encrypted, over-limit, checking, or unattested supplemental PDFs.
4. Export is allowed for warning-state PDFs after user attestation.
...
6. A shared filing-eligibility guard rejects missing, stale, blocked, unattested, digest-mismatched, or legacy-only uploaded records at the model/finalizer boundary.
...
1. Should warning-state documents require a stronger second confirmation than cleanly parsed documents?
2. Should the app allow a document with no extractable text if the filer attests, or should that be a hard block for court technology-standard reasons?
3. Should warning-state documents count against a separate risk summary on the final review screen?
4. Should attestation be per file, per schedule, or one final filing-level certification that lists all supplemental PDFs?
5. Should final packet metadata include a note that supplemental pages were user-supplied and preserved as uploaded?
