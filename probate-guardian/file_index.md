...
| [`src\core\persistence\case-file.js`](<src/core/persistence/case-file.js>) | Case-file persistence: builds the .sav archive (buildCaseFileBlob(), format version 1; a password-protected file names no ward outside its encrypted parts), saves it (Save Backup, silent auto-save to the remembered file, download fallback), keeps the remembered file handle, imports and restores backups (importSavArchiveOrWard()), refuses a file in a newer format (newerCaseFileFormatMessage()), and protectPartiallyReadCaseFile(), which tells the filer which parts of a file could not be read and stops that file being saved over. |
...
| [`src\core\types\window-bridge.d.ts`](<src/core/types/window-bridge.d.ts>) | Generated by scripts/audit-window-bridge.mjs --declare: declares every name a module assigns to or reads from window (typed any) so tsc --noEmit can check those modules; tests/unit/window-bridge.spec.js fails when it is out of date with the source. |
...
| [`tests\e2e\case-file-damaged-open.spec.ts`](<tests/e2e/case-file-damaged-open.spec.ts>) | A case file with a part that cannot be read (a filing's entry missing, the case records unreadable), on each way in -- the startup screen's Open, Open backup, and the re-read after an unlock: the filer is told exactly what was not read, the rest opens, and auto-save never writes to the damaged original; a control proves the harness sees the write for a sound file. |
| [`tests\e2e\case-file-manifest-privacy.spec.ts`](<tests/e2e/case-file-manifest-privacy.spec.ts>) | A password-protected case file names no ward anywhere outside its encrypted parts (every entry read as text), still opens with every filing by name, and an unencrypted file keeps the names in its manifest. |
| [`tests\e2e\case-file-newer-format.spec.ts`](<tests/e2e/case-file-newer-format.spec.ts>) | A case file marked with a newer format version is refused on the startup screen, by Open backup, and on the re-read after an unlock, and never becomes the file auto-save writes to. |
...
| [`tests\unit\fixtures\window-bridge-allowlist.json`](<tests/unit/fixtures/window-bridge-allowlist.json>) | Checked-in allow-list of every window.X = assignment in the source, by file; tests/unit/window-bridge.spec.js fails on any assignment not listed, so each new global is added deliberately. |
