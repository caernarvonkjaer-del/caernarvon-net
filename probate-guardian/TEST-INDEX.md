...
| signature-capture.contract.spec.ts       | Milestone 39-B: three-state signature control (Unsigned/"/s/"/Stamp) on Plan Simplified's Guardian card (pilot); Milestone 39-C: rolled out to Plan Annual/Initial/Minor, Simplified Accounting, and Annual/Final/Trust Accounting's Guardian, Attorney, Preparer, and Certificate-of-Service Attorney cards, incl. per-card stamp-image-paint proof via pdf.js's operator list; the Upload background-transparency luminance-threshold fix proved once through a real file upload |
