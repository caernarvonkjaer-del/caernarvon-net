...
| navigation-status.contract.spec.ts       | Guardian Inventory nav/status contract; Annual/Final/Trust, Simplified, and Plan-type field-path accuracy (Milestone 33 item 3, sub-phases 3b/3c/3d); Milestone 39-C: checkSignatureState() jump-link wording ("date signed"/"signature stamp image") resolution across every filing type |
...
| routes.spec.ts                           | Route table behavior; sidebar nav accordion forgets a hand-opened section on navigation (Milestone 40F)                                                                                  |
