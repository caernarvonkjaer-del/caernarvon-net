...
| party-resolver.spec.ts                   | party-resolver hydration/dehydration core; party de-duplication via resolver (Milestone 43C: retitled describe block to disambiguate from party-dedupe.spec.ts's identical title); Milestone 40C-1: single-ward import reconstruction under unanimity rule, legacy backfill and merge conflict handling; Milestone 49: near-name candidate tier, mergeRecord written on merge, unmergeParty() full undo (edited-since-merge fields kept), one-level-only sub visibility, pre-tracking tombstones never listed    |
