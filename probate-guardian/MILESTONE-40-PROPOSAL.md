...
| 40F | Unify the duplicate save/autosave/export pipeline (`legacy-app.js` vs. `case-file.js`), fix its false "Last backup" indicator bugs, and remove the inert Tauri desktop scaffolding (filesystem ward-backup, OS-keychain "remember password") | **Landed in full 2026-09-13** (`9ac92dd`, `4ad99c1`, `619cfd8`): boot `ReferenceError` fixed, one save clock, failure escalation centralized, Tauri scaffolding removed, and the legacy duplicates deleted (net −640 lines) once 40G unblocked Step 4 | `MILESTONE-40F-PROPOSAL.md` |
...
| `src/core/pdf/pdf-engine.js` | **40C-1**, 40E | 40C-1 edits the county caption fallback (`:166`); 40E adds an array branch to `measureCell()` (`:1143`). Different functions, ~1000 lines apart. | Either. Lowest-risk pairing in the set — noted only so a merge conflict in this file isn't mistaken for a scope collision. |
