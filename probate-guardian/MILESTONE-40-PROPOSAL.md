...
| 40H | Dashboard guardian-validation crash guard, Safe Deposit Box conditional data loss, D-3 `fieldset`/`legend` accessibility gap, and a malformed `<summary>` on the four Plan types' Print Preview readiness panel | Draft, ready for approval; no file overlap with 40C-1. Also documents (not scoped) a larger recommendation: unify the two non-unified readiness-panel architectures split across filing-type families | `MILESTONE-40H-PROPOSAL.md` |
